...
packages_a_installer <- c("tidyverse","readr","ggplot2","ggpubr","dplyr",
              ...
              "crosstable","flextable","knitr","kableExtra","pROC","htmlTable")
...
# on regarde quelles variables sont correlées et à quoi 
...
#un tableau descriptif et un de moyennes conditionnelles 
...
#création de boxplot pour certaines variables intéressantes à creuser
...
#on s'intéresse aux wards placées en particulier : 
...
#on fait un ttest pour être sur de  cette intuition
...
#test si le côté de jeu influe ou non sur la victoire 
...
#on peut faire un ttest sur le reste des variables (pas anova car seulement victoire et défaite donc même résultat à ttest et anova)
...
rm(resultats_tt,resultats_tt2,results_t_tests,conf_level,
...
#avec une graine de reproductibilité pour avoir les mêmes résultats que dans le rapport
set.seed(777)
...
#on a également enlevé les variables qui expriment la même information (tour, cs, JGCS ...)
...
# performance du model (ROC, AUC) puis matrice de confusion
...
#on donne un nom à nos 2 modalités (on considère predict > 0.5 -> victoire)
#et on leur associe des intervalles 
#on aurait pu choisir de définir plus de catégories afin d'être plus précis mais cela serait moins cohérent avec l'objectif
{Loose_predict <- test$predict_logit >= 0 & test$predict_logit < 0.5
Win_predict <- test$predict_logit >= 0.5 & test$predict_logit <= 1}
  
#on reporte ces valeurs dans notre data set pour voir à quel catégorie de 
#possibilité de victoire le modèle l'aurait associé 
test$ProbWin <- ifelse(Loose_predict, "Loose","Win") 

#on vérifie le modèle aini obtenu en comparant les valeurs attendues aux valeurs
#obtenues réellement (matrice de confusion)
...
#AUC-ROC ; accuracy ; recall ;F1 score ; facilité d'interprétation
...
#on récupère les résultats qu'on met dans des vecteurs 
...
#on crée un data frame avec nos vecteurs pour en faire un tableau par la suite 
...
choix_du_modèle = c(
ifelse(auc_roc[1] > auc_roc[2], "Modèle Logit", ifelse(auc_roc[1] < auc_roc[2], "Modèle Probit", "Égalité")),
...
coeff_log_exp$Coeff <- log(coeff_log_exp$`exp(coeff_log)`)
coeff_log_exp$Effet <- ifelse(coeff_log_exp$Coeff> 0 , "Positif", "Négatif")
coeff_log_exp <- coeff_log_exp %>%
  mutate(var_odd_ratio = paste0(round((exp(coeff_log) - 1) * 100, 3), "%"))

#on rajoute nos significativités (basées sur le stargazer plus haut)
...
#on les rend un peu plus jolies avec des icones
coeff_log_exp <- coeff_log_exp %>% 
  mutate(Sign_Icon = ifelse(Signif == "***", "✅✅✅",
                            ifelse(Signif == "**", "✅✅", 
                                   ifelse(Signif=="*", "✅", "❌"))))
#on réordonne le tableau pour le rendre plus lisible et on enlève l'intercept
#qu'on analysera plus tard
coeff_log_exp <- coeff_log_exp %>% 
  select(Coeff,Sign_Icon, Effet, `exp(coeff_log)`, var_odd_ratio) 
coeff_log_exp1 <- coeff_log_exp %>% 
slice(-1)

#on fait un tableau qui parle visuellement avec des couleurs etc. 
# et note de bas pour expliquer les icones (au cas où)
coeff_log_exp1 %>%
  kable(format = "html") %>%
  kable_styling(full_width = FALSE, position = "center") %>%
  row_spec(which(coeff_log_exp1$Coeff < 0), background = "#ffcccb") %>%
  row_spec(which(coeff_log_exp1$Coeff > 0), background = "#d3f8d3") %>% 
  row_spec(which(coeff_log_exp1$Sign_Icon=="❌"), background = "lightgrey")%>%
  add_header_above(c(" " = 1, "Coefficients du modèle Logit" = 5)) %>%
...
#on pourrait se demander nos chances de victoire si toutes les variables sont à 0 ? 
